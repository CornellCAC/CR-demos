...
\begin{frame}
\frametitle{Kernel-based C/R}

Requires kernel modules or kernel patches to run.

% CRIU, BLCR and OpenVZ.
% CRIU is in mainline kernel.
% BLCR does not work for newer kernels (past 3.7.1). 
...
Checkpoint one or several interacting processes. Does not usre the full container model.
% requires API
...
\begin{frame}
\frametitle{Library-wrapper C/R}
% BLCR and DMTCP
% DMTCP similar in function to BLCR, but doesn't require kernel module.
